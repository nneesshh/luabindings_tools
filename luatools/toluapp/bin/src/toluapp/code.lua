...
function classCode:register(pre)
	...
	-- get first line
	local _, _, first_line = strfind(self.text, "^([^\n\r]*)")
	if strfind(first_line, "^%s*%-%-") then
		if strfind(first_line, "^%-%-##") then
			first_line = strgsub(first_line, "^%-%-##", "")
			if flags["C"] then
				s = strgsub(s, "^%-%-##[^\n\r]*\n", "")
			end
		end
 ...
 --]]
	...
	local b =
		...
		function(c)
			...
			return strformat("%3u,%s", strbyte(c), e)
		end
	)
	output(b .. strbyte(" "))
	output("\n" .. pre .. " };\n")
