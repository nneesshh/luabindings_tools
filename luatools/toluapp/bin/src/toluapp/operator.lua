...
function classOperator:supcode_tmp()
	...
	output("#ifndef TOLUA_RELEASE\n")
	output(
		'  if (!self) tolua_error(tolua_S,"' .. output_error_hook("invalid 'self' in function '%s'", self.name) .. '",NULL);'
	)
	output("#endif\n")
...
function Operator(d, k, a, c)
	local op_k = strgsub(k, "^%s*", "")
	op_k = strgsub(k, "%s*$", "")
	--if strfind(k, "^[%w_:%d<>%*%&]+$") then
	...
	local btype, bctype = isbasic(l[1].type)
	if k == "[]" and (l[1] == nil or (btype ~= "integer" and btype ~= "number")) then
