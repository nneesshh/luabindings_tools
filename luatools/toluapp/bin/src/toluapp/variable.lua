...
function classVariable:cfuncname(prefix)
	...
	name = prefix .. parent .. unsigned .. "_" .. strgsub(self.lname or self.name, ".*::", "") .. ptr
...
-- get variable value
function classVariable:getvalue(class, is_static, prop_get)
	local name
	if prop_get then
		name = prop_get .. "()"
	else
...
function classVariable:getpointervalue(class, is_static)
...
function classVariable:supcode()
	...
	if strfind(self.mod, "tolua_property") then
		local _, _, type = strfind(self.mod, "tolua_property__([^%s]*)")
		type = type or "default"
		prop_get, prop_set = get_property_methods(type, self.name)
		self.mod = strgsub(self.mod, "tolua_property[^%s]*", "")
	end
	...
	-- return value
	if strfind(self.mod, "tolua_inherits") then
	...
	-- set function ------------------------------------------------
	if not (strfind(self.type, "const%s+") or strfind(self.mod, "tolua_readonly") or strfind(self.mod, "tolua_inherits")) then
		if class then
