...
function classContainer:supcode()
	...
	-- check class sigleton
	if self.classtype == 'class' then
		local i = 1
		while self[i] do
			if self[i].mod then
				local _, _, is_static = strfind(self[i].mod, "^%s*(static)")
				if is_static then
					class_singleton_hook(self)
					break
				end
			end
			i = i + 1
...
function pop()
	...
	--for k,v in pairs(classContainer.curr.usertypes) do print(k,v) end
...
-- substitute typedef
function applytypedef(mod, type_)
	return classContainer.curr:applytypedef(mod, type_)
end

-- check if is type
function findtype(type_)
	local t = classContainer.curr:findtype(type_)
	return t
end

-- check if is typedef
function istypedef(type_)
	return classContainer.curr:istypedef(type_)
end
...
-- check if is enum
function isenum(type_)
	return classContainer.curr:isenum(type_)
end
...
function classContainer:overload(lname)
	...
	return strformat("%02d", self.lnames[lname])
end

-- applies typedef: returns the 'the facto' modifier and type
function classContainer:applytypedef(mod, type_)
	if global_typedefs[type_] then
		--print("found typedef "..global_typedefs[type_].type)
		local mod1, type1 = global_typedefs[type_].mod, global_typedefs[type_].ftype
...
function classContainer:istypedef(type_)
	local env = self
	while env do
		if env.typedefs then
			local i = 1
			while env.typedefs[i] do
				if env.typedefs[i].utype == type_ then
					return type_
				end
				i = i + 1
			end
...
function find_enum_var(var)
	...
	while c do
		...
		for k, v in pairs(_global_enum_values) do
...
function match_type(childtype, regtype, st)
	...
	local b, e = strfind(regtype, childtype, -strlen(childtype), true)
	if b then
		if
			e == strlen(regtype) and
				(b == 1 or (strsub(regtype, b - 1, b - 1) == ":" and strsub(regtype, 1, b - 1) == strsub(st, 1, b - 1)))
		 then
			return true
		end
	end
...
function classContainer:isenum(type_)
	if global_enums[type_] then
		return type_
	end

	local basetype = strgsub(type_, "^.*::", "")
	local env = self
	while env do
		if env.enums then
			local i = 1
...
function classContainer:parseAnonymousBlock(block_)
	local b_, e_, body_, decl_

	-- strip "{}"
	local sv = strsub(block_, 2, -2)
	sv = strgsub(sv, "^%s*", "");
	sv = strgsub(sv, "%s*$", "");

	while strlen(sv) > 0 do
		-- anonymous struct in union
		b_, e_, body_ = strfind(sv, "^%s*struct%s*(%b{})[^;]*;")
		if b_ then
			-- try struct
			local sv2 = strsub(body_, 2, -2)
			self:parseAnonymousBlock(sv2)
		else
			-- array
			b_, e_, decl_ = strfind(sv, "^%s*([_%w][_@%s%w%d%*&:<>]*%[[_%w%d]*%])%s*;%s*")
			if b_ then
				-- try array
				Array(decl_)
			else
				...
				if b_ then
					-- try variable
					local list_ = split_c_tokens(decl_, ",")
					Variable(list_[1])
					if list_.n > 1 then
						local _, _, type_ = strfind(list_[1], "(.-)%s+([^%s]*)$")

						local i = 2
						while list_[i] do
							Variable(type_ .. " " .. list_[i])
							i = i + 1
						end
					end
				end
...
function classContainer:doparse(s)
	...
	do
		local b, e, code = strfind(s, "^%s*;")
		if b then
			return strsub(s, e + 1), strsub(s, 1, e)
		end
	end
	...
	do
		local b, e, code = strfind(s, "^%s*(%b\5\6)")
		if b then
			code = strsub(code, 2, -2) .. "\n"
			Verbatim(code, "")
			return strsub(s, e + 1), strsub(s, 1, e)
	...
	do
		local b, e = strfind(s, "^%s*TOLUA_PROTECTED_DESTRUCTOR%s*;?")
		if b then
	...
	-- try 'extern' keyword
	do
		local b, e = strfind(s, "^%s*extern%s+")
		if b then
			-- do nothing
			return strsub(s, e + 1), strsub(s, 1, e)
		end
	...
	do
		local b, e = strfind(s, "^%s*virtual%s+")
		if b then
	...
	do
		local b, e = strfind(s, "^%s*%w*%s*:[^:]")
		if b then
			return strsub(s, e), strsub(s, 1, e - 1) -- preserve the [^:]
		end
	end
	...
	-- try define const: "const static type variable = xyz;"
	do
		local b, e, vartype, varname = strfind(s, "^%s*const%s+static%s+([^%s]+)%s+([^%s]+)%s*=?[^;]*;%s*")
		if b then
			_curr_code = strsub(s, b, e)
			DefineConst(vartype, varname)
			return strsub(s, e + 1), strsub(s, 1, e)
		end
	end
	...
	do
		...
		if not b then
			...
			b, e, decl, kind, arg, const =
				strfind(s, "^%s*([_%w][_%w%s%*&:<>,]-[%s%*&]+operator)%s*([^%s][^%s]*)%s*(%b())%s*(c?o?n?s?t?)[%s\n]*%b{}%s*;?%s*")
		end
		if not b then
			-- try cast operator
			b, e, decl, kind, arg, const = strfind(s, "^%s*(operator)%s+([%w_:%d<>%*%&%s]+)%s*(%b())%s*(c?o?n?s?t?)")
			if b then
				local _, ie = strfind(s, "^%s*%b{}", e + 1)
				if ie then
					e = ie
				end
			end
	...
	do
		...
		if not b then
			...
			if b then
				decl = strgsub(decl, "%(%s*%*([^%)]*)%s*%)", " %1 ")
			end
		end
		if b then
			if virt and strfind(virt, "[=0]") then
				if self.flags then
	...
	do
			...
			if b then
				...
				if b then
					...
					if b then
						...
						b, e, name, base, body = strfind(s, "^%s*union%s*([_%w@]*)%s*([^{]-)%s*(%b{})[^;]*;")
						if b then
							-- union
							if "" == name then
		...
		if b then
			if base ~= "" then
				...
				base = strgsub(base, "^%s*:%s*", "")
				base = strgsub(base, "%s*public%s*", "")
				base = split(base, ",")
			else
				base = {}
			end
			...
			if not anonymous_union then
				Class(name, base, body)
				if not dummy then
					varb, vare, varname = strfind(s, "^%s*([_%w]+)%s*;", e + 1)
					if varb then
			...
			elseif body then
				-- anonymous union, declare it as variable list
				self:parseAnonymousBlock(body)
			end
			return strsub(s, e + 1), strsub(s, 1, e)
		end
	end
	...
	do
		...
		if b then
			...
			Variable(list[1])
			if list.n > 1 then
				local _, _, type_ = strfind(list[1], "(.-)%s+([^%s]*)$")

				local i = 2
				while list[i] do
					Variable(type_ .. " " .. list[i])
					i = i + 1
				end
			end
			--Variable(decl)
	...
	-- no matching
	local remain = strgsub(s, "%s%s*", "")
	if remain ~= "" then
		_curr_code = s
		--error("#parse error")
		print("\n>[classContainer:doparse()] parse error: unknown code pattern, remain=", remain)
	else
		return ""
	end
end
