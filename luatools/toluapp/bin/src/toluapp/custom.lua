...
function include_file_hook(p, filename)
	...
	p.code = strgsub(p.code, "\n%s*SigC::Signal", "\n\ttolua_readonly SigC::Signal")
	p.code = strgsub(p.code, '#ifdef __cplusplus\nextern "C" {\n#endif', "")
	p.code = strgsub(p.code, "#ifdef __cplusplus\n};?\n#endif", "")
	p.code = strgsub(p.code, "DECLSPEC", "")
	p.code = strgsub(p.code, "SDLCALL", "")
	p.code = strgsub(p.code, "DLLINTERFACE", "")
	p.code = strgsub(p.code, "#define[^\n]*_[hH]_?%s*\n", "\n")
