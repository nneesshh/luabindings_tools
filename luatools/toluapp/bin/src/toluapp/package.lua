...
function classPackage:preprocess()
    ...
    self.code = strgsub(self.code, "/%*", "\1")
    self.code = strgsub(self.code, "%*/", "\2")
    self.code = strgsub(self.code, "%b\1\2", "")
    self.code = strgsub(self.code, "\1", "/%*")
    self.code = strgsub(self.code, "\2", "%*/")
    self.code = strgsub(self.code, "%s*@%s*", "@") -- eliminate spaces beside @
    self.code = strgsub(self.code, "%s?inline(%s)", "%1") -- eliminate 'inline' keyword
    --self.code = strgsub(self.code,"%s?extern(%s)","%1") -- eliminate 'extern' keyword
    --self.code = strgsub(self.code,"%s?virtual(%s)","%1") -- eliminate 'virtual' keyword
    --self.code = strgsub(self.code,"public:","") -- eliminate 'public:' keyword
    self.code = strgsub(self.code, "([^%w_])void%s*%*", "%1_userdata ") -- substitute 'void*'
    self.code = strgsub(self.code, "([^%w_])void%s*%*", "%1_userdata ") -- substitute 'void*'
    self.code = strgsub(self.code, "([^%w_])char%s*%*", "%1_cstring ") -- substitute 'char*'
    self.code = strgsub(self.code, "([^%w_])wchar_t%s*%*", "%1_wcstring ") -- substitute 'wchar_t*'
    self.code = strgsub(self.code, "([^%w_])lua_State%s*%*", "%1_lstate ") -- substitute 'lua_State*'
...
function classPackage:preamble()
    ...
    if self:requirecollection(_collect) then
        ...
        collect_function_hook(_collect)
        output("#endif\n\n")
    end
    ...
    repeat
        code, nsubst =
            ...
            function(kind, fn, extra)
                ...
                elseif kind == "i" then
                    local t = {code = s}
                    extra = strgsub(extra, "^%s*,%s*", "")
                    local pars = split_c_tokens(extra, ",")
                    include_file_hook(t, fn, tunpack(pars))
                    return "\n\n" .. t.code
                else
...
function prep(file)
    ...
    for line in file:lines() do
        if strfind(line, "^##") then
            table.insert(chunk, strsub(line, 3) .. "\n")
        else
            local last = 1
            for text, expr, index in strgmatch(line, "(.-)%$(%b())()") do
                last = index
                if text ~= "" then
                    table.insert(chunk, strformat("table.insert(__ret, %q )", text))
                end
                table.insert(chunk, strformat("table.insert(__ret, %s )", expr))
            end
            table.insert(chunk, strformat("table.insert(__ret, %q)\n", strsub(line, last) .. "\n"))
        end
    end
