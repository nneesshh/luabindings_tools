		...
		output(
			pre ..
				'tolua_cclass(tolua_S,"' ..
					self.lname .. '","' .. self.type .. '","' .. self.btype .. '",' .. _collect[self.type] .. ");"
		)
		output(pre, "#else\n")
...
function classClass:requirecollection(t)
	...
	if self._delete or ((not flags["D"]) and self._new) then
		--t[self.type] = "tolua_collect_" .. strgsub(self.type,"::","_")
...
function classClass:decltype()
	push(self)
	self.type = register_usertype_hook(self.original_name or self.name)
	self.btype = get_usertype_hook(self.base)
	self.ctype = "const " .. self.type
	if self.extra_bases then
		for i = 1, #self.extra_bases do
			self.extra_bases[i] = get_usertype_hook(self.extra_bases[i])
		end
	end
	local i = 1
	while self[i] do
...
function Class(n, p, b)
	...
	-- check for template
	b = strgsub(b, "^{%s*TEMPLATE_BIND", "{\nTOLUA_TEMPLATE_BIND")
	local t, _, T, I = strfind(b, '^{%s*TOLUA_TEMPLATE_BIND%s*%(+%s*"?([^",]*)"?%s*,%s*([^%)]*)%s*%)+')
	if t then
		-- remove quotes
		I = strgsub(I, '"', "")
		T = strgsub(T, '"', "")
		...
		local bs = strgsub(b, "^{%s*TOLUA_TEMPLATE_BIND[^\n]*\n", "{\n")
