...
int main(int argc, char* argv[])
{
	...
	{
		// 
		const char *toluapp_path = "src/toluapp/";
		char  path[BUFSIZ];
		char  main_entry[BUFSIZ];
		char* p;

		// concat path from argv[0] and toluapp_path
		strcpy(path, argv[0]);
		...
		// main entry
		strcpy(main_entry, path);
		strcat(main_entry, "main.lua");
		if (luaL_dofile(L, main_entry)) {
			fprintf(stderr, "dofile %s error : %s", main_entry, lua_tostring(L, -1));
		}
	}
#endif
